...
import { rowParted, rowCenter, Space, column, expand, fullHeight, center, row } from "@worktools/flex-styles";
...
export let useConfirmPop = (options?: IConfirmOptions) => {
  ...
  let renderIcon = () => {
    switch (confirmOptions?.type) {
      case "error":
        return (
          <span className={cx(styleIcon, styleError)}>
            <XCircle />
          </span>
        );
      case "warning":
      default:
        return (
          <span className={cx(styleIcon, styleWarning)}>
            <AlertCircle />
          </span>
        );
    }
